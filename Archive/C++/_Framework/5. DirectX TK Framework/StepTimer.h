...
// StepTimer: CPU의 성능 카운터를 바탕으로 게임 시간을 관리합니다. Tick과 프레임 사이의 간격을 측정하고, update문을 호출합니다.
// * DirectX TK 파일을 바탕으로 작성되었습니다.
...
namespace DX
{
	...
	public:
		...
		void Tick(const TUpdate& update)
		{
			...
			if (mIsFixedTimeStep)
			{
				if (std::abs(static_cast<long long>(deltaTick - mTargetElapsedTicks)) < TicksPerSecond / static_cast<ull>(FixedDeltaTickClampSensitivity))
				{
