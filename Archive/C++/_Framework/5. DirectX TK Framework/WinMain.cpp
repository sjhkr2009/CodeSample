...
namespace
{
	...
	public:
		...
		void Initialize(HWND hwnd, int width, int height) override
		{
			...
			m_gameObject = DX::ActorManager::Instance().Create<DX::AnimatedActor>(
				static_cast<int>(Layer::Character),
				L"Assets/spriteSheet.png",
				L"Assets/spriteSheet.json",
				L"Assets/animation.json"
			);
			m_gameObject->SetAnimation(L"Move");
			m_gameObject->SetPosition(100.f, 100.f);

			auto pBackground = DX::ActorManager::Instance().Create<DX::Actor>(
	...
	protected:
		void Update(DX::StepTimer const& timer) override
		{
			...
			m_gameObject->Update(timer.GetElapsedSeconds());
			...
			if (state.D2)
			{
				m_gameObject->SetAnimation(L"Die_Effect");
			}
			if (state.D3)
			{
				m_gameObject->SetAnimation(L"Die");
			}
		}
		//-------------------------------------------------
	};

	std::unique_ptr<MyGame> gGame;
}
