...
void Game::CreateDeviceDependentResources()
{
	...
	// CreateWICTextureFromFile: 파일명을 입력하면 WIC를 통해 변환한 후, 리소스 및 리소스 뷰에 반환합니다.
	//	ㄴ 리소스는 원본 리소스의 정보(ex: float, float, float), 리소스 뷰는 특정 목적(화면 표시)을 위해 변환된 정보(ex: r,g,b 또는 x,y,z)
	//	ㄴ 여기서는 텍스쳐 자체에 접근할 일은 없으니 리소스 뷰만 받는다.
	...
	// 참고: 한 번에 불러올 수 있는 이미지 크기에는 제한이 있다. 
	//		(DirectX 9에서는 2048x2048, 11에서는 16384x16384. 단, PC는 DirectX 지원 여부만 보면 되지만, 휴대폰은 기종에 따라 최대치가 정해져 있어서 4096 x 4096 등 최소사양 기준으로 자른다.)
}
